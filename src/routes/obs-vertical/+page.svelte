<script lang="ts">
	...
	function preloadProgram(program: Program | null) {
		...
		for (const screen of program.graphics ?? []) {
			...
			// Prefer vertical alt path for preloading
			const rawPath = screen.graphics_path_vertical ?? screen.graphics_path;
			const url = imgUrl(rawPath);
			if (url) preload(url, getType(rawPath));
		}
	...
	async function triggerPopUp(li: number, payload: PopUpPayload) {
		...
		while (popupPending[li]) {
			...
			// Clamp popup width to the 1080px portrait canvas (24px edge padding on each side).
			const VP = 24;
			const maxW = window.innerWidth - 2 * VP;
			const clampedW = Math.min(naturalW, maxW);
			const clampedH = clampedW < naturalW ? Math.round(naturalH * (clampedW / naturalW)) : naturalH;
			...
			if (target.direction === "bottom") {
				...
				container.style.left = `max(${VP}px, min(calc(100vw - ${clampedW + VP}px), calc(${pos}% - ${halfW}px)))`;
			} else if (target.direction === "top") {
				container.style.top = "30px";
				container.style.left = `max(${VP}px, min(calc(100vw - ${clampedW + VP}px), calc(${pos}% - ${halfW}px)))`;
			} else if (target.direction === "left") {
				container.style.left = `${VP}px`;
				container.style.top = `calc(${pos}% - ${halfH}px)`;
			} else {
				container.style.right = `${VP}px`;
				container.style.top = `calc(${pos}% - ${halfH}px)`;
			}
	...
	// ── Popup payload builder (uses vertical overrides when available) ─────────
	function buildPopUpPayload(pa: import("$lib/types").ProgramPopUp): PopUpPayload | null {
		...
		// Prefer vertical overrides for direction/position
		const direction = ((popup.direction_vertical ?? popup.direction) ?? "bottom") as Direction;
		const position = popup.position_vertical ?? popup.position ?? 50;
		if (popup.media_type === "html") {
			if (!popup.html_content) return null;
			return {
				src: null, type: "html", html: popup.html_content,
				direction, position, width: w, height: h,
			};
		}
	...
	onMount(() => {
		...
		function onStudioState(data: StudioState) {
			...
			// Blank all overlay layers first
			for (let li = 0; li < NUM_LAYERS; li++) {
				show(li, null, "image");
			}

			// Restore active overlays — use vertical alt path when available
			for (const overlay of data.activeOverlays ?? []) {
				...
				if (mediaType === "html") {
					...
					const rawPath = overlay.graphicPathVertical ?? overlay.graphicPath ?? null;
					const url = rawPath ? imgUrl(rawPath) : null;
			...
			// Restore active popups — use vertical overrides when available
			for (const popup of data.activePopUps ?? []) {
				const li = (popup.layer ?? 1) - 1;
				fillerSuppressed[li] = true;
				const mediaType = (popup.mediaType ?? "image") as MediaType;
				const direction = ((popup.directionVertical ?? popup.direction) ?? "bottom") as Direction;
				const position = popup.positionVertical ?? popup.position ?? 50;
				if (mediaType === "html") {
					const html = popup.htmlContent ?? null;
					if (html) triggerPopUp(li, { src: null, type: "html", html, direction, position, width: popup.width ?? null, height: popup.height ?? null });
				} else {
					const rawPath = popup.imagePath ?? null;
					const url = rawPath ? imgUrl(rawPath) : null;
					const type = getType(rawPath);
					if (url) preload(url, type);
					if (url) triggerPopUp(li, { src: url, type, html: null, direction, position, width: popup.width ?? null, height: popup.height ?? null });
				}
			}
		...
		function onOverlayActivated(data: any) {
			...
			if (mediaType === "html") {
				...
				// Prefer vertical alt path
				const rawPath: string | null = data.graphicPathVertical ?? data.graphicPath ?? null;
				const url = rawPath ? imgUrl(rawPath) : null;
				const type = getType(rawPath);
				if (url) preload(url, type);
				show(li, url, type);
		...
		function onPopUpStarted(data: any) {
			...
			// Use vertical overrides when present
			const direction = ((data.directionVertical ?? data.direction) ?? "bottom") as Direction;
			const position = data.positionVertical ?? data.position ?? 50;
			...
			if (mediaType === "html") {
				...
				fillerSuppressed[li] = true;
				stopFillers(li);
				triggerPopUp(li, { src: null, type: "html", html, direction, position, width: popupW, height: popupH });
			} else {
				...
				fillerSuppressed[li] = true;
				stopFillers(li);
				triggerPopUp(li, { src: url, type, html: null, direction, position, width: popupW, height: popupH });
			}
		}
