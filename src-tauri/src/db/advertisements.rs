...
pub fn get_all_ads(conn: &Connection) -> Result<Vec<Advertisement>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, sponsor_name, comments, media_path, media_type, direction, position, created_at FROM advertisements ORDER BY id"
    )?;
...
pub fn get_ad(conn: &Connection, id: i64) -> Result<Option<Advertisement>> {
    let result = conn.query_row(
        "SELECT id, name, sponsor_name, comments, media_path, media_type, direction, position, created_at FROM advertisements WHERE id = ?1",
        [id],
        |row| Ok((
            row.get::<_, i64>(0)?,
            row.get::<_, String>(1)?,
    ...
    match result {
        Ok((aid, name, sponsor_name, comments, media_path, media_type, direction, position, created_at)) => {
...
pub fn create_ad(conn: &Connection, name: &str, sponsor_name: &str, comments: &str, direction: &str, position: i64) -> Result<Advertisement> {
    conn.execute(
        "INSERT INTO advertisements (name, sponsor_name, comments, direction, position) VALUES (?1, ?2, ?3, ?4, ?5)",
...
pub fn update_ad(conn: &Connection, id: i64, name: &str, sponsor_name: &str, comments: &str, direction: &str, position: i64) -> Result<Option<Advertisement>> {
    let rows = conn.execute(
        "UPDATE advertisements SET name = ?1, sponsor_name = ?2, comments = ?3, direction = ?4, position = ?5 WHERE id = ?6",
