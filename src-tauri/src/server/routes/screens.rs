...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/upload-image-vertical", post(upload_screen_image_vertical))
...
async fn upload_screen_image_vertical(
    State(state): State<AppState>,
    mut multipart: Multipart,
) -> impl IntoResponse {
    let mut file_bytes: Option<Vec<u8>> = None;
    ...
    let filename = format!("v_{}.{}", Uuid::new_v4(), file_ext);
    let abs_path = abs_dir.join(&filename);
    if let Err(e) = tokio::fs::write(&abs_path, &bytes).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({ "error": e.to_string() }))).into_response();
    }
    ...
    let db = state.db.lock().await;
    let rel_path_clone = rel_path.clone();
    match tokio::task::block_in_place(|| crate::db::screens::set_media_path_vertical(&db, id, &rel_path_clone)) {
        Ok(_) => {
            {
                let io_clone = state.io.lock().ok().and_then(|g| g.clone());
                if let Some(io) = io_clone {
